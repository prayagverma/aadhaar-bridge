...
public class AadhaarOtpAsyncTask extends AsyncTask<String, Void, OtpResponse> {
    ...
    OtpCaptureData otpData;

    public AadhaarOtpAsyncTask(Context ctx, ABSEvent event, OtpCaptureData otpCaptureData) {
        this.otpData = otpCaptureData;
        this.ctx = ctx;
        this.event = event;
    }
    ...
    protected OtpResponse doInBackground(String... params) {
        ...
        try {
            StringEntity entity = new StringEntity(GsonSerializerUtil.marshall(otpData));
            Log.d(LOG_TAG, "REQUEST : " + GsonSerializerUtil.marshall(otpData));
