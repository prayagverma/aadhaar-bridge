...
public class AadhaarBfdAsyncTask extends AsyncTask<String, Void, BfdResponse> {
    ...
    protected static final String LOG_TAG = AadhaarBfdAsyncTask.class.getSimpleName();
	protected ProgressDialog mDialog;
	protected Context ctx;
    protected ABSEvent event;

	BfdCaptureData bfdData;

	public AadhaarBfdAsyncTask(Context ctx, ABSEvent event, BfdCaptureData bfdCaptureData) {
		this.bfdData = bfdCaptureData;
		this.ctx = ctx;
        this.event = event;
	}
	...
	@Override
	protected BfdResponse doInBackground(String... params) {
		...
		try {
			StringEntity entity = new StringEntity(GsonSerializerUtil.marshall(bfdData));
            Log.d(LOG_TAG, "REQUEST : " + GsonSerializerUtil.marshall(bfdData));
		...
		try {
			...
			if (response != null && response.getStatusLine().getStatusCode() == 200) {
				...
				return GsonSerializerUtil.unmarshal(responseContent, BfdResponse.class);
			} else {
	...
	private BfdResponse buildErrorResponse() {
		BfdResponse response = new BfdResponse();
